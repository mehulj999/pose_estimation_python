...
def left_arm_bicep_curl_tracker(running, stats):
    """Guided left arm bicep curl tracker with on-screen instructions and countdown."""
    mp_pose = solutions.pose
    pose = mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5)
    ...
    print("🎥 Running guided left arm bicep curl detector — Press 'q' in the video window to stop")
        ...
        if results.pose_landmarks:
            ...
            if (shoulder_idx < len(landmarks) and elbow_idx < len(landmarks) and wrist_idx < len(landmarks) and nose_idx < len(landmarks)):
                ...
                if phase == "show_face_and_arm":
                    show_instruction = "Show your face and your whole left arm."
                    if ready_to_count:
                        phase = "straighten"
                elif phase == "straighten":
                    show_instruction = "Straighten your left arm (angle 140-180)."
                    if STRAIGHT_ANGLE_MIN <= left_angle <= STRAIGHT_ANGLE_MAX:
                    ...
                    cv2.circle(frame, left_elbow, 10, (255, 0, 0), -1)
                    cv2.putText(frame, f"Angle: {int(left_angle)}", (left_elbow[0]+15, left_elbow[1]),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 51), 2)
                    if awaiting_curl:
                        if left_angle < CURL_ANGLE_MAX:
                            stats["reps"] += 1
                            phase = "down"
                            curl_started = True
                            rep_counted = True
                            ...
                            rep_counted = False
                            awaiting_curl = False
                    elif left_angle > STRAIGHT_ANGLE_MIN:
                        phase = "straighten"
                        countdown_start = None
                        awaiting_curl = False
                elif phase == "down":
                    show_instruction = "Move your arm down to start again."
                    if left_angle > STRAIGHT_ANGLE_MIN:
                        phase = "straighten"
                        countdown_start = None
                        awaiting_curl = False
                if not ready_to_count:
                ...
                rep_counted = False
                awaiting_curl = False
                show_instruction = "Landmarks not found. Show your face and left arm."
        else:
            phase = "show_face_and_arm"
            countdown_start = None
            curl_started = False
            rep_counted = False
            awaiting_curl = False
            show_instruction = "Waiting for pose... Show your face and left arm."
